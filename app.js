...
async function startRazorpaySubscription() {
  dismissMobileKeyboard();
  showSpinner("Initializing secure subscription gateway...");

  try {
    // 1. Call Cloudflare Backend function to generate subscription ID
    const response = await fetch(`${CONFIG.apiUrl}`, {
      method: "POST",
      headers: { "Content-Type": "text/plain" },
      body: JSON.stringify({ action: "createSubscription" })
    });
    ...
    if (data.status !== "success" && !data.id) {
      showToast(data.message || "Failed to initialize subscription", true);
      return;
    }
    ...
    const options = {
      ...
      "handler": function (response) {
        showToast("Software Subscription Successful!");
